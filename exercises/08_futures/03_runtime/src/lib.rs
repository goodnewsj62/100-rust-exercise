    ...
    T: Display + Send + Sync + 'static,
{
    ...
    tokio::spawn(accept_stream(first, Arc::clone(&reply)));
    tokio::spawn(accept_stream(second, reply));
}

async fn accept_stream<T>(listener: TcpListener, reply: Arc<T>) -> Result<(), anyhow::Error>
where
    T: Display + Send + Sync + 'static,
{
    loop {
